type chip8cpu
	V as integer[16]
	disp as integer[2048]
	mem as integer[4096]
	stack as integer[24]
	keystate as integer[16]
...
global chip8_fontset as integer[80] = [
...
function chip8emu_exec_cycle(cpu ref as chip8cpu)
	...
	select (cpu.opcode && 0xF000)
		...
		case 0x7000: /* 7XNN: Adds NN to VX */
			...
			cpu.V[X] = cpu.V[X] + (cpu.opcode && 0x00FF)
			cpu.pc = cpu.pc + 2
		endcase
		...
		case 0xC000: /* CXNN: Vx=rand() && NN */
			cpu.V[(cpu.opcode && 0x0F00) >> 8] = Mod(Random(), (0xFF + 1)) && (cpu.opcode && 0x00FF)
			cpu.pc = cpu.pc + 2
		endcase
		...
		case 0xF000:
			select (cpu.opcode && 0x00FF)
				...
				case 0x001E: /* FX1E: Add VX to I register */
					cpu.I = cpu.I + cpu.V[(cpu.opcode && 0x0F00) >> 8]
					cpu.pc = cpu.pc + 2
				endcase
